...
> Status: **Complete; all six phases verified on 2026-09-12.** Installation,
> Hyprland, desktop component, operational, hardware, troubleshooting, and
> publishing references reflect the pinned v1.5.0 sources.
>
...
| R2 | Domain inconsistency (`.org` vs `.dev`) | High | Resolved in Phase 6: canonical signals use `hyprflux.dev`. |
...
| R7 | Broken Kitty sidebar route | High | Resolved in Phase 4 with a source-validated Kitty page. |
| R8 | Empty animation page | Medium | Resolved in Phase 3 from `user-animations.lua`. |
...
| R10 | Historical posts contain superseded architecture | Low | Resolved in Phase 6 with archive banners and current-guide handoffs. |
| R11 | Missing social preview image | Medium | Resolved in Phase 6 by using the maintained `logo.webp` asset. |
...
| R13 | pnpm build entrypoint is blocked by an unresolved esbuild approval | Medium | Resolved in Phase 6 with an explicit `allowBuilds.esbuild: true` policy. |
...
| 6 | [Integrate Navigation, Metadata, Drift Checks, and Final QA](./phase-06-integrated-qa.md) | Complete; verified 2026-09-12 |
...
Use the release checklist in `README.md` and run `pnpm docs:check` whenever the
HyprFlux source or a public route changes. Review the source-defect backlog
separately in HyprFlux; the documentation intentionally describes those risks
without changing upstream behavior.
